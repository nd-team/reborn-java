...
import com.bjike.reborn.common.jpa.boot.JpaCache;
...
public class AppJpaCache implements JpaCache {
    ...
    public List<Cache> initCaches() {
        ConcurrentMapCache serCache = new ConcurrentMapCache("warehouseSerCache");
        serCache.put("timeToLiveSeconds", 60 * 60);//1小时过期
        serCache.put("timeToIdleSeconds", 60 * 60 * 12);//闲置时间

        ConcurrentMapCache daoCache = new ConcurrentMapCache("warehouseDaoCache");
